...
export const createCategory = async (req, res) => {
    try {
        ...
        // if (!req.files || !req.files.categoryImage === 0) {
        ...
        // }
        const categoryImage = "https://images.squarespace-cdn.com/content/v1/62ec1ff71a3945114dc3c287/1681763230299-CSLTFH492FGT1ML3RCMD/654-010+Roll-on.jpg";
        ...
        res.status(200).json({ success: true, category, message: "category created" });
    } catch (err) {
...
export const editCategory = async (req, res) => {
    try {
        ...
        category = await Category.findByIdAndUpdate({ _id: categoryId }, categoryData);
...
export const getAllCategory = async (req, res) => {
    try{
        ...
        if(!categories){
            return res.status(400).json({success:false, message:"Categories not available"});
        }

        res.status(400).json({success:true,categories,message:"Here is your category list"});
